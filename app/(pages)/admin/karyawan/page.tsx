...
const EmployeeManagement = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Perbaikan: Gunakan PUT untuk update jika PATCH bermasalah di server
    const url = editingId 
    ...
    try {
      ...
      if (!res.ok) {
        // Tangkap pesan error dari API jika ada (misal: NIK sudah ada)
        const errorMsg = result.message || (result.errors ? Object.values(result.errors).flat().join(", ") : "Gagal memproses data.");
  ...
  const handleEdit = (item: Karyawan) => {
    ...
    setFormData({
      ...
      status_aktif: item.status_aktif == true || item.status_aktif == 1 ? "1" : "0"
    });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4">
            <div className={`bg-white rounded-3xl shadow-sm border p-6 sticky top-6 transition-all duration-300 ${editingId ? 'border-blue-400 ring-4 ring-blue-50' : 'border-gray-100'}`}>
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${editingId ? 'bg-blue-600 text-white' : 'bg-indigo-50 text-indigo-600'}`}>
                ...
                {editingId && (
                  <button onClick={resetForm} className="bg-gray-100 p-2 rounded-lg text-gray-500 hover:text-red-500 transition-colors">
                    <X size={16} />
                  </button>
                )}
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button 
                  disabled={loading}
                  className={`w-full text-white font-bold py-3 rounded-xl transition-all shadow-lg mt-4 flex items-center justify-center gap-2 ${editingId ? 'bg-blue-600 hover:bg-blue-700 shadow-blue-200' : 'bg-slate-900 hover:bg-slate-800 shadow-slate-200'}`}
                >
                  {loading ? <Loader2 className="animate-spin" size={18} /> : (editingId ? "Update Data Karyawan" : "Simpan Data Karyawan")}
                </button>
              </form>
            </div>
          </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="p-6 border-b border-gray-50 flex justify-between items-center bg-white">
                ...
                <div className="flex gap-2">
                   <button onClick={fetchData} className="text-xs bg-gray-50 hover:bg-gray-100 px-3 py-1 rounded-full font-bold text-slate-500 transition-colors">
                    Refresh
                  </button>
                  <span className="text-xs bg-indigo-50 px-3 py-1 rounded-full font-bold text-indigo-600">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-gray-50/30">
                      <th className="px-6 py-4">Karyawan & Info</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {karyawanList.map((item) => (
                      <tr 
                        key={item.id} 
                        className={`hover:bg-gray-50/80 transition-all group ${editingId === item.id ? 'bg-blue-50/50' : ''}`}
                        onMouseEnter={() => setHoveredRow(item.id)}
                        onMouseLeave={() => setHoveredRow(null)}
                      >
                        <td className="px-6 py-5">
                          <div className="font-bold text-slate-800">{item.nama}</div>
                          <div className="text-[10px] text-slate-400 font-black tracking-tight uppercase">NIK: {item.nik}</div>
                        ...
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-bold text-slate-700">
                              {item.jabatan?.jabatan || jabatanList.find(j => j.id === item.id_jabatan)?.jabatan || "Tidak Ada"}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                            item.status_aktif == 1 || item.status_aktif == true ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"
                          }`}>
                            {item.status_aktif == 1 || item.status_aktif == true ? "Aktif" : "Non-Aktif"}
                          </span>
                        </td>
